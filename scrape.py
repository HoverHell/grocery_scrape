...
class WorkerBase:
    ...
    def __init__(self):
        ...
        self.reqr = requests.Session()  # TODO: retries
...
class WorkerUtk(WorkerBase):
    ...
    def process_cat_page(self, cat, page):
        ...
        if page_resp.status_code == 301:  # pages over limit redirect to non-paged `url2`
    ...
    def process_item(self, item_bs, page_resp):
        ...
        for price_el in prices_el.select('.goods_price-item.current'):
            ...
            value = float(self.el_text(price_el).replace(',', '.'))
